...
#--- READ WRF DATA
    filenames = glob.glob(inputdir+"/"+'wrf_wind_Hovin.txt')
    np_wrf_array_list = []
...
# ----- OBS    
    station_B = big_frame.loc[big_frame['station_id'] == 18210]  
    #print(station_B)
...
#--- Scatterplot
    ...
    for label, bfw in big_frame_wrf.groupby('gridbox', sort=False):
        ...
        for height, bfww in bfw.groupby('height', sort=False):
            outfilename='Hovin_wind_'+label+str(height)+'.png'
            if height == 25:
    				#print(bfww['Speed'])
                    bfwww = pd.concat([station_B['speed'], bfww['Speed']], axis=1)
                    bfwww.columns = bfwww.columns.str.replace('Speed','WRF')
                    bfwww.columns = bfwww.columns.str.replace('speed','Observations')
                    bfwww = bfwww.dropna()
                    bfwww.plot.scatter(figsize=(10,10), x='Observations', y='WRF', c=color, grid=True )#, title='Comparison of wind speed data at {}m - {}'.format(height,label))
    				#plt.title=('Comparison of temperature data at {}m'.format(height), fontsize=20)
