...
#A quick way to create new windrose axes...
    ...
    for label, bfw in big_frame_wrf.groupby('gridbox', sort=False):
        for height, bfww in bfw.groupby('height', sort=False):
            outfilename='Munch_rose_'+label+str(height)+'.png'
            if height == 10:
                ax = new_axes()
                ax.bar(bfww['Direction'], bfww['Speed'], opening=0.8, edgecolor='white', bins=np.arange(0.5, 15, 2))
                plt.title('Wind Direction and Intensity at {}m - {}'.format(height,label), y=1.08, fontsize=16, fontweight='bold')
                ax.set_xlim(0,450)
                ax.set_ylim(0,450)
                ...
                ax.bar(bfww['Direction'], bfww['Speed'], opening=0.8, edgecolor='white', bins=np.arange(0.5, 15, 2))
                plt.title('Munch - WRF -  {}m - {}'.format(height,label), y=1.08, fontsize=16, fontweight='bold')
                ax.set_xlim(0,450)
                ax.set_ylim(0,450)
                ax.yaxis.set_ticks(np.arange(0,450,500))
                ax.yaxis.set_ticklabels(np.arange(0,450,500))
                ax.set_legend()
                ax.legend(loc='center left', bbox_to_anchor=(1, 0.3), title='Value [m/s]', fontsize=10) 
