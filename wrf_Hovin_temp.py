...
#--- READ WRF DATA
    filenames = glob.glob(inputdir+"/"+'wrf_temp_Hovin.txt')
    np_wrf_array_list = []
...
# ----- OBS
    ...
    station_2 = big_frame.loc[big_frame['station_id'] == 18210] 
    station_2 = station_2['T_2m'] 
 
    station_10 = big_frame.loc[big_frame['station_id'] == 18210]
...
#--- Scatterplot
    ...
    for label, bfw in big_frame_wrf.groupby('gridbox', sort=False):
        for height, bfww in bfw.groupby('height', sort=False):
            outfilename='Hovin_temp_'+label+str(height)+'.png'
            if height == 2:
